...
import os
import cv2
from scripts.color_align import adjust_tensor_image_to_gt
...
class RealESRNetModel(SRModel):
    ...
    def feed_data(self, data):
        ...
        if self.is_train and self.opt.get('high_order_degradation', True):
            ...
            if self.opt['color_align']:
                self.gt = adjust_tensor_image_to_gt(self.gt, self.opt['color_diff_dict']).to(self.device)

            # ----------------------- The first degradation process ----------------------- #
            # blur
            out = filter2D(self.gt, self.sinc_kernel)
            out = self.gt
            # random resize
